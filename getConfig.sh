#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`sls-there-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`sls-there-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
